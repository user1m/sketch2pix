...
if [ "$1" != "--data-root" ]; then
    ...
    echo "usage: ./test.sh --data-root ../dataset/celebfaces/face2edge --name edge2face_generation --direction BtoA"
    exit 1;
#elif [ "$3" != "--phase" ]; then
...
time (env DATA_ROOT=$2 name=$4 which_direction=$6 th test.lua)
